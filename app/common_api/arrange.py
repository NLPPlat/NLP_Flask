...
# 管道列表获取
@api.route('/arrange/arranges', methods=['GET'])
@jwt_required
def arrangeFetch():
    # 读取基本数据
    info = request.values
    queryArrangeName = info.get('arrangeName')
    ...
    if len(queryArrangeName) > 0:
        q = q & Q(arrangeName__icontains=queryArrangeName)
    ...
    # 数据库查询
    arrangesList = Arrange.objects(q).order_by(sort)
    ...
    if type == 'part':
        ...
        return {'code': RET.OK, 'data': {'total': arrangesList.count(), 'items': arrangesList[front:end]}}
    else:
        return {'code': RET.OK, 'data': {'total': arrangesList.count(), 'items': arrangesList}}
